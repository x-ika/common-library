...
public final class ImageProcessor {
    ...
    public void buildRects(int[] c, int d, int n, List<ImageRect> l) {
        System.arraycopy(ZERO, 0, B, 0, S);
        System.arraycopy(ZERO, 0, T, 0, S);
        for (int p = 0; p < S; p++) {
            if (B[p] == 0 && c[p] > 0) {
    ...
    private ImageRect create(int p, int q) {
        return new ImageRect(I[p], J[p], I[q] + 1, J[q] + 1);
    }
    ...
    public void drawRects(int[] c, List<ImageRect> list, int color) {
        for (ImageRect r : list) {
