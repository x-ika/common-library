...
public final class EnglishNumberToWords {
    ...
    public static String convert(String decimalNumber) {
        ...
        long integerPart, fractionalPart;
        int zeroCount = 0;

        if (decimalNumber.indexOf('.') == -1) {
            integerPart = Long.parseLong(decimalNumber);
            fractionalPart = 0;
        } else {
            String[] parts = decimalNumber.split("[.]");
            integerPart = Long.parseLong(parts[0]);
            fractionalPart = Long.parseLong(parts[1]);
            zeroCount = parts[1].length() - String.valueOf(fractionalPart).length();
        }

        return fractionalPart == 0 ?
                convert(integerPart) :
                convert(integerPart) + " point " + StringUtils.repeat("zero ", zeroCount) + convert(fractionalPart);
